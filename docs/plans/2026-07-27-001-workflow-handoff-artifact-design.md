...
需要先纠正一个常见误解：**当前实现已经把父需求全文复制进了每个节点子 issue**。

`workflowTaskIssueDescription`（`server/internal/handler/workflow_instance.go:1403`）把节点说明和 `host.Description` 拼接后写入子 issue 的 `description`：

```go
parts = append(parts, instructions)   // 模板说明或节点说明
parts = append(parts, requirement)    // 父 issue 描述全文
```

所以"智能体看不到父需求"在当前代码下不成立。真正的问题是相反的一面：

- 父需求在每个子 issue 中形成副本，父需求更新后副本不会跟随。
- 工作流越长，副本越多，且互相之间可能不一致。
- 后续节点需要读取前序节点的结论，如果默认继承全部评论，上下文会持续放大。
...
子 issue 的 `description` 是人和智能体共用的同一个字段。直接删除父需求拼接对智能体有利（无副本、不会过时），但成员打开子 issue 后只剩一句节点说明，是体验倒退。

因此采用引用：既消除副本，又保留人工可达性。
...
- 更新制品会创建新版本。
- 下游默认读取当前最新版本。
- 已经开始执行的节点记录当时看到的版本 ID，便于复现。
- 审批绑定具体版本；制品更新后需要重新审核。

### 9.5 存储决策：独立制品实体

制品正文使用独立实体，不复用 issue 附件。

原因是 §9.4 的三条要求 issue 附件都无法满足：附件没有版本链，无法把审批绑定到具体版本，也无法记录"下游节点当时读到的是哪一版"。这三条是制品的核心语义，缺一条方案就不成立。

制品需要记录：制品 ID、所属工作流运行、来源节点运行、名称和说明、类型、当前版本、提交人和提交时间、审核状态。
...
节点可以配置 `max_attempts`，默认 3。

上限约束的是"产出未交齐后重新执行"和"审核驳回后重做"这两类循环。达到上限后节点进入阻塞，等待人工介入，不再自动重新派发。

没有上限会产生一个真实的失控路径：智能体反复结束执行却始终交不齐必需制品，节点在"缺产出 → 重新执行"之间无限循环，持续消耗额度且没有人被通知。§16 的状态机在补上这一项之前是没有出口的。

人工介入后的可选动作是：放宽产出要求、人工补齐产出并完成、跳过节点、或取消工作流。计数在人工解除阻塞后重置。
...
stateDiagram-v2
    ...
    MissingOutput --> Working: 智能体继续补充，尝试次数未达上限
    MissingOutput --> Blocked: 尝试次数达到上限
    MissingOutput --> AwaitingReview: 人工补充完成
    Working --> AwaitingReview: 产出齐全且需要审核
    Working --> Completed: 产出齐全且无需审核
    AwaitingReview --> Working: 审核驳回，尝试次数未达上限
    AwaitingReview --> Blocked: 审核驳回且尝试次数达到上限
    AwaitingReview --> Completed: 审核通过
    Blocked --> Working: 人工解除阻塞，计数重置
    Blocked --> Completed: 人工补齐产出并完成
...
`Blocked` 复用现有的 `blocked` 状态。这里与前一版的判断不同：`blocked` 的语义是"节点无法靠自身推进，需要外部介入"，尝试次数耗尽正好符合这个语义。区分"外部依赖阻塞"和"尝试耗尽"用阻塞原因字段表达，不需要两个状态——两者的处理路径最终都是同一件事：通知人，等人做决定。

`MissingOutput → Working` 和 `AwaitingReview → Working` 两条回边都受 §11.5 的 `max_attempts` 约束。这两条是状态机中仅有的环，不加约束就没有出口。
...
1. **子 issue metadata 不变量**。零风险，且是第 3、4 步的前提。
2. **`max_attempts` 与阻塞出口**。独立于其余各项，但它堵的是一条会持续消耗额度的失控路径，越早越好。
3. **父需求改为引用**。相对独立，可单独验证。
4. **`multica workflow` 命令组 + 制品实体**。依赖第 1 步。
5. **`node_choice` 与 `api` 审核方式**。必须早于第 6 步。
6. **移除 `submission_schema.fields` 与表单构建器**。影响面最大，放在最后。

两条硬约束：

- 第 5 步和第 6 步倒序会导致条件分支断档，不可调换。
...
- **Given**：节点配置 `max_attempts = 3`，且要求提交《技术方案.md》。
- **When**：智能体连续三次结束执行都没有提交该制品。
- **Then**：节点进入阻塞并给出原因，不再自动重新派发；额度停止消耗，相关人员收到通知。
...
- 触及 `max_attempts` 上限进入阻塞的节点比例，以及阻塞后人工选择的动作分布。
- `api` 审核方式的采用率、超时率，以及因端点不可达被判定为阻塞的比例。
- 工作流节点 issue 在 issue 列表中的占比，以及用户对列表噪音的反馈量（见 §15.2）。
...
- CLI 命令集是否有缺口或冗余。
- `max_attempts` 默认值 3 是否合适。
- 节点物化成 issue 这个前提是否需要重新评估（见 §15.2）。
...
4. 工作流终止后，制品保留周期是否跟随父 issue。

这些问题可以先采用保守默认值，再根据 §23 的使用数据调整。

原第 5 项"制品存储复用 issue 附件还是独立实体"已在 §9.5 确定为独立实体，不再作为待确认项。
