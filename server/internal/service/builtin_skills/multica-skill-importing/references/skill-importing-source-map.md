...
| Behavior | File:line |
|---|---|
| `detectImportSource` | `server/internal/handler/skill.go` (`grep -n "func detectImportSource"`) |
| Atlas Skill Hub (Lilith host + `skill-hub` path segment) | `server/internal/handler/skill.go` (matcher `isAtlasSkillHubURL`, `isLilithHost`) |
| `skills.sh` / `www.skills.sh` | `server/internal/handler/skill.go` (`detectImportSource`) |
| `clawhub.ai` / `www.clawhub.ai` | `server/internal/handler/skill.go` (`detectImportSource`) |
| `github.com` / `www.github.com` | `server/internal/handler/skill.go` (`detectImportSource`) |
| Bare slug (no host) defaults to ClawHub | `server/internal/handler/skill.go` (`detectImportSource`) |
