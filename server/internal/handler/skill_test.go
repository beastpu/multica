...
func TestFetchFromAtlasSkillHub_ExtractsBundle(t *testing.T) {
	...
	result, err := fetchFromAtlasSkillHub(context.Background(), server.Client(), rawURL)
	if err != nil {
...
func TestFetchFromAtlasSkillHub_SurfacesJSONError(t *testing.T) {
	...
	_, err := fetchFromAtlasSkillHub(context.Background(), server.Client(), server.URL+"/api/skill-hub-internal/lark-cli/install-prompt")
	if err == nil {
