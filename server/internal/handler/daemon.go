...
func (h *Handler) ClaimTasksByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	for i := range claimed {
		...
		resp, deliveredCommentIDs, _, _, failure := h.buildClaimedTaskResponse(r, &task, rt, uuidToString(task.RuntimeID), rtWorkspaceID, req.SupportsChatAsk)
		if failure != nil {
...
func (h *Handler) buildClaimedTaskResponse(r *http.Request, task *db.AgentTaskQueue, runtime db.AgentRuntime, runtimeID, runtimeWorkspaceID string, supportsChatAsk bool) (resp AgentTaskResponse, deliveredCommentIDs []pgtype.UUID, agentSkillCount, builtinSkillCount int, failure *claimBuildFailure) {
		...
		if task.IsLeaderTask && task.SquadID.Valid && resp.Agent != nil {
			...
			if parseErr == nil {
				if squad, squadErr := h.Queries.GetSquadInWorkspace(
					...
					// A workflow direct task has no issue at all — the node is
					// the record, and its status follows the run. Telling the
					// leader it owns an issue status would name authority it
					// has nowhere to exercise.
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	resp, deliveredCommentIDs, agentSkillCount, builtinSkillCount, failure := h.buildClaimedTaskResponse(r, task, runtime, runtimeID, runtimeWorkspaceID, claimReq.SupportsChatAsk)
	if failure != nil {
		outcome = failure.outcome
		writeError(w, failure.status, failure.message)
		return
