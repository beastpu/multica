...
// keys, so every reference is detached or deleted explicitly.
func cleanupWorkflowRelationshipsForIssue(ctx context.Context, q *db.Queries, issue db.Issue) error {
	if err := q.DetachWorkflowNodeTasksByIssue(ctx, db.DetachWorkflowNodeTasksByIssueParams{
		IssueID: issue.ID, WorkspaceID: issue.WorkspaceID,
	}); err != nil {
		return fmt.Errorf("detach workflow task from issue: %w", err)
	}
	...
	if err := q.DetachWorkflowIssuesByHost(ctx, hostParams); err != nil {
		return fmt.Errorf("detach workflow issues from host: %w", err)
	}
	...
	if err := q.DeleteWorkflowRoleAssignmentsByHost(ctx, db.DeleteWorkflowRoleAssignmentsByHostParams{
		...
		return fmt.Errorf("delete workflow role assignments: %w", err)
	}
