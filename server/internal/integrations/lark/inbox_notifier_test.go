...
func TestInboxNotifierPatchesExistingMergedIssueCardForLifecycleUpdate(t *testing.T) {
	workspaceID := mustUUID("11111111-1111-1111-1111-111111111111")
	userID := mustUUID("22222222-2222-2222-2222-222222222222")
	actorAgentID := mustUUID("44444444-4444-4444-4444-444444444444")
	issueID := mustUUID("55555555-5555-5555-5555-555555555555")
	...
	for _, want := range []string{"✅ Issue 已创建", "🔄 状态已变更", "`待办` → `处理中`"} {
...
func TestInboxNotifierSendsNewCommentAsFreshCardEvenWhenMergedIssueCardExists(t *testing.T) {
	workspaceID := mustUUID("11111111-1111-1111-1111-111111111111")
	userID := mustUUID("22222222-2222-2222-2222-222222222222")
	actorAgentID := mustUUID("44444444-4444-4444-4444-444444444444")
	issueID := mustUUID("55555555-5555-5555-5555-555555555555")
	...
	if q.issueCardArg.WorkspaceID.Valid || q.touchCardID.Valid {
		t.Fatalf("new_comment should not read or patch merged issue card")
	}
	api.mu.Lock()
	defer api.mu.Unlock()
	if len(api.patches) != 0 {
		t.Fatalf("new_comment should not patch existing card, got patches=%d", len(api.patches))
	}
	if len(api.directCardsOut) != 1 {
		t.Fatalf("expected one fresh direct card for new_comment, got %d", len(api.directCardsOut))
	}
	card := api.directCardsOut[0].CardJSON
	for _, want := range []string{"💬 Agent 评论", "今天上海天气：白天阵雨转多云。"} {
		if !strings.Contains(card, want) {
			t.Fatalf("fresh comment card missing %q: %s", want, card)
		}
	}
}
