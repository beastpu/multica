...
// chat session. It is best-effort: the Router calls it after the message is
// durable, and the ingested message itself remains authoritative.
...
func (r *Router) processClaimed(ctx context.Context, set ResolverSet, msg channel.InboundMessage, inst ResolvedInstallation, claimToken pgtype.UUID) (Result, dedupFinalize, error) {
	...
	// Text preemption for structured asks: this durably-appended message
