...
// writeAvailableCommandsP4Assessment keeps read-only assessment tasks away from
// the ordinary issue workflow commands that can mutate user-visible state.
func writeAvailableCommandsP4Assessment(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("Use the `multica` CLI only for read-only assessment evidence. Do not use issue create/update/status/comment commands in this task.\n\n")
	b.WriteString("### Core\n")
	if ctx.P4AssessmentBindingID != "" {
		fmt.Fprintf(b, "- `multica api get /api/operations/agent-fixes/%s/p4-evidence` — fetch P4/Swarm assessment evidence.\n", ctx.P4AssessmentBindingID)
	} else {
		b.WriteString("- `multica api get /api/operations/agent-fixes/<binding-id>/p4-evidence` — fetch P4/Swarm assessment evidence.\n")
	}
	b.WriteString("\n")
}
...
func writeAlwaysUseCLIReadOnly(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `multica` CLI\n\n")
	b.WriteString("Access Multica platform resources only through the read-only `multica` CLI path above — never `curl` / `wget`. If evidence is unavailable or the CLI lacks a needed read operation, return conservative `unknown` values with warnings instead of mutating platform state.\n\n")
}
...
//	----------------------+-----------+---------+--------+-----------+--------------+------
//	Available Commands    | evidence  |   full  |  full  |   full    |   minimal    | full
//	Comment Formatting    |     —     |    ✓    |   ✓    |     —     |      —       |  —
//	Repositories          |     —     |    △    |   △    |     △     |      —       |  △
//	Project Context       |     —     |    △    |   △    |     △     |      △       |  △
//	Issue Metadata        |     —     |    ✓    |   ✓    |     —     |      —       |  —
...
func buildMetaSkillContentSlim(provider string, ctx TaskContextForEnv) string {
	...
	switch kind {
	case kindP4Assessment:
		writeAvailableCommandsP4Assessment(&b, ctx)
	case kindQuickCreate:
	...
	if kind == kindP4Assessment {
		writeAlwaysUseCLIReadOnly(&b)
	} else {
		writeAlwaysUseCLI(&b)
	}
