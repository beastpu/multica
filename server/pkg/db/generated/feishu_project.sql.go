...
func (q *Queries) GetFeishuProjectIntegration(ctx context.Context, workspaceID pgtype.UUID) (FeishuProjectIntegration, error) {
	...
	err := row.Scan(
		...
		&i.WorkItemTypes,
		&i.SyncOnlyWorkspaceMemberItems,
	)
	return i, err
}
...
SET project_key = $3,
    ...
    updated_at = now()
WHERE id = $1 AND workspace_id = $2
RETURNING id, workspace_id, project_key, plugin_id, plugin_secret, actor_user_key, enabled, sync_story, sync_issue, mql_filter, status_mapping, reverse_status_mapping, created_by_id, last_synced_at, last_error, created_at, updated_at, assign_open_items_to_owner_agent, business_line_field_key, business_line_field_name, last_seen_updated_at_ms, label_sync_rules, last_orphan_reconciled_at, work_item_types, sync_only_workspace_member_items
`
...
func (q *Queries) UpdateFeishuProjectIntegrationByID(ctx context.Context, arg UpdateFeishuProjectIntegrationByIDParams) (FeishuProjectIntegration, error) {
	row := q.db.QueryRow(ctx, updateFeishuProjectIntegrationByID,
		...
		arg.LabelSyncRules,
		arg.WorkItemTypes,
		arg.SyncOnlyWorkspaceMemberItems,
		arg.ActorUserKey,
	)
	var i FeishuProjectIntegration
	err := row.Scan(
