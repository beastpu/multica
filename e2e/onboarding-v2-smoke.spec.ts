...
test("onboarding v2 — welcome → role → use_case (skip path)", async ({ page }) => {
  ...
  // Continue to the first persisted question.
  ...
  await page.screenshot({ path: `${SHOTS_DIR}/02-role.png` });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({ path: `${SHOTS_DIR}/03-use-case.png` });
  ...
  await page.screenshot({ path: `${SHOTS_DIR}/04-workspace.png` });
});

test("onboarding v2 — rage-skip all questions", async ({ page }) => {
  const api = new TestApiClient();
...
test("onboarding v2 — zh-Hans renders Chinese labels", async ({ page, context, baseURL }) => {
  await context.addCookies([
    { name: "multica-locale", value: "zh-Hans", url: baseURL ?? "http://localhost:3000" },
  ]);
  const api = new TestApiClient();
  ...
  // Role screen — Chinese question
  await expect(page.getByText("你是什么角色？")).toBeVisible({ timeout: 10000 });
  await page.waitForTimeout(500);
  await page.screenshot({ path: `${SHOTS_DIR}/07-role-zh.png` });
});
