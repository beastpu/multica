...
| **Live reply card** | Short tasks reply directly. After about 7 seconds, longer tasks post a live card with the current stage, elapsed time, and action counts; the final answer replaces that progress in place. |
...
<Callout type="info">
The progress card only shows public-safe stage names and aggregate counts. It never exposes the agent's private reasoning, raw commands, file paths, or tool output.
</Callout>
...
- Once the agent starts writing, its answer streams into a card and keeps filling in until the run finishes. This uses CardKit and needs the Bot app scope `cardkit:card:write`; an admin can grant it from **Manage in Lark**. Apps without it fall back to an ordinary updateable card — same answer, no typewriter. A reply that arrives before the agent narrates anything is sent as a normal message, with no card at all.
