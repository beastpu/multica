...
- A run that is still going after a few seconds posts a progress card that reports how long it has been working, and is replaced in place by the answer. It needs no scope beyond the ones binding already grants. If that card cannot be updated, the answer is sent as an ordinary message instead rather than being dropped.
