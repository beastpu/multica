...
- CardKit's native streaming effect requires the Bot app scope `cardkit:card:write`. Existing apps without that scope automatically fall back to an ordinary updateable card; an admin can grant it from **Manage in Lark**.
