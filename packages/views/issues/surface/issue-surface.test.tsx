...
describe("IssueSurface — scope switch loading semantics", () => {
  ...
  it("routes empty-state creation through a surface-specific callback", async () => {
    ...
    setApiInstance({
      ...
      // The list surface now proves its window through server status branches,
      // whose total comes from the facets query. A pending facets query leaves
      // the total unknown, and "unknown" is deliberately not "empty" — so the
      // empty state needs a resolved, zero-total facet response to appear.
      listIssueTableFacets: vi.fn(() =>
        Promise.resolve({ query_fingerprint: "fp", total: 0, facets: [] }),
      ),
      listGroupedIssues: vi.fn(() => never()),
      listProjects: vi.fn(() => never()),
      getAgentTaskSnapshot: vi.fn(() => never<AgentTask[]>()),
