...
export function useIssueSurfaceData({
  ...
  const listFilter = useMemo<IssueListFilter | undefined>(() => {
    const nextFilter: IssueListFilter = {
      priorities: priorityFilters,
      assignee_filters: assigneeFilters,
      include_no_assignee: includeNoAssignee,
      creator_filters: creatorFilters,
  ...
  const loadMoreFilter = useMemo<MyIssuesFilter | undefined>(() => {
    if (!queryPlan.loadMoreFilter) return undefined;
    return listFilter
      ? { ...listFilter, ...queryPlan.loadMoreFilter }
      : queryPlan.loadMoreFilter;
  }, [listFilter, queryPlan.loadMoreFilter]);
  ...
  const statusIssuesQuery = useQuery({
    ...issueSurfaceListOptions(wsId, queryPlan, sort, listFilter),
    enabled:
      !usesAssigneeBoard &&
      !usesGantt &&
      !usesTable &&
  ...
  const bucketedIssues = useMemo(() => {
    return serverStatusBranches.enabled
      ...
      : serverGroupBranches.enabled
        ...
        : usesAssigneeBoard
          ? (assigneeGroupsQuery.data?.groups.flatMap(
              (group) => group.issues,
            ) ?? [])
