  ...
  return (
    <article className="space-y-3 rounded-xl border bg-surface px-4 py-3">
      <div className="flex items-start justify-between gap-3">
        ...
        {/*
          The badge reads "issue created" for `materialized`, which is what
          materialising a template task means. A critic task materialises no
          issue, so the badge announced one that does not exist — next to a
          node whose real issue belongs to a different task on the same card
          list.
        */}
        {task.materialization_status === "materialized" && !task.issue_id
          ? null
