...
export function issueSurfaceListOptions(
  ...
  const scopedFilter: MyIssuesFilter =
    plan.kind === "scoped" && filter
      ? { ...filter, ...plan.queryFilter }
      : plan.queryFilter;

  return (
    plan.kind === "workspace"
      ? issueListOptions(wsId, sort, filter)
      : myIssueListOptions(wsId, plan.queryScope, scopedFilter, plan.userId, sort)
