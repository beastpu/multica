...
describe("contentReferencesAttachment", () => {
  ...
  it("matches when markdown uses a content or download URL variant", () => {
    const full = {
      ...att,
      content_url: "https://cdn.example.test/uploads/workspaces/ws/legacy.png?inline=1",
      download_url: "https://cdn.example.test/api/attachments/123/download",
    };
    expect(contentReferencesAttachment(`![file](${full.content_url})`, full)).toBe(true);
    expect(contentReferencesAttachment(`![file](${full.download_url})`, full)).toBe(true);
  });

  it("matches equivalent relative and absolute URL paths", () => {
    const full = {
      ...att,
      content_url: "https://cdn.example.test/uploads/workspaces/ws/legacy.png?inline=1",
    };
    const md = "![file](/uploads/workspaces/ws/legacy.png?inline=1)";
    expect(contentReferencesAttachment(md, full)).toBe(true);
  });
